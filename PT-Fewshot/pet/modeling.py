...
def train_pet(
    ...
    for pattern_id in pattern_ids:  # ---------------------------对于每一个PVP
        for iteration in range(repetitions):  # -----------------重复实验：进行训练与测试
            ...
            """
            加载模型， 模型为TransformerModelWrapper类的对象，主要属性: .model: prompt_encoder,
            """
            ...
            if do_eval:
                ...
                save_predictions(
                    os.path.join(pattern_iter_output_dir, "dev32_predictions.jsonl"),
                    wrapper,
                    dev32_result,
                )
                save_logits(
                    os.path.join(pattern_iter_output_dir, "dev32_logits.txt"),
                    dev32_result["logits"],
                )
    ...
    if do_eval:
        ...
        _write_results(
            os.path.join(output_dir, "result_test.txt"), results, dev32_results
        )
    else:
...
def _write_results(path: str, all_results: Dict, dev32_results: Dict):
    with open(path, "w") as fh:
        ...
        for metric in dev32_results.keys():
            all_results = [
                result
                for pattern_results in dev32_results[metric].values()
                for result in pattern_results
            ]
            all_mean = statistics.mean(all_results)
            all_stdev = statistics.stdev(all_results) if len(all_results) > 1 else 0
